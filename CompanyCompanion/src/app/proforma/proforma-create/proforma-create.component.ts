...
export class ProformaCreateComponent implements OnInit {
  ...
  SaveProforma() {
    if (this.proformaForm.valid) {
      this.service.SaveProforma(this.proformaForm.getRawValue()).subscribe(res => {
        if (this.isEdit) {
  ...
  CustomerChange() {
    ...
    this.service.GetCustomerByCode(customerCode).subscribe(res => {
      ...
      customData = res;
      if (customData != null) {
        this.proformaForm.get("customerDeliveryAddress")?.setValue(customData.address + ' ,' + customData.phone + ' ,' + customData.email)
        this.proformaForm.get("customerName")?.setValue(customData.name)
      }
    })
  }
  ShowProformaNumber() {
    this.isGeneratedShow = this.proformaForm.get("isGenerated")?.value as boolean;
  }
